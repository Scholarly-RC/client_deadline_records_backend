...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.WARNING("Starting file migration to R2..."))
        ...
        for doc in documents:
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"  ❌ Error processing {doc.document_file.name}: {str(e)}"
                    )
                )
                error_count += 1
        ...
        if dry_run:
            self.stdout.write(
                "\n"
                + self.style.WARNING(
                    "This was a dry run. Use --dry-run=False to perform actual migration."
                )
            )
            ...
            if not delete_local:
                self.stdout.write(
                    self.style.WARNING(
                        "Local files were kept. Use --delete-local to remove them after migration."
                    )
                )
